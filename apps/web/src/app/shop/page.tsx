...
const metrics = [
  { label: "카테고리", value: shopCategories.length },
  { label: "추천 상품", value: shopCategories.reduce((total, category) => total + category.items.length, 0) },
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <PageHeader
          eyebrow={<Badge tone="brand">Creator shop</Badge>}
          title="카테고리별 상점"
          description="트렌드 콘텐츠에 바로 연결하기 좋은 상품을 카테고리별로 정리했습니다. 각 상품은 외부 검색 페이지로 연결되어 탐색으로 이어집니다."
          action={
          ...
          <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm" key={category.title}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <h2 className="mt-3 text-2xl font-bold text-ink">{category.title} 상점</h2>
