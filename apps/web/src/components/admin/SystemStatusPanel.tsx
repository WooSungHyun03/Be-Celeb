...
import { testLLM, testYouTube } from "@/lib/client/admin-api";
...
export function SystemStatusPanel({ status, apiBaseUrl, onError }: SystemStatusPanelProps) {
  ...
  async function runTest(type: "youtube" | "llm") {
    setIsTesting(true);
    try {
      const result = type === "youtube" ? await testYouTube() : await testLLM();
      if (type === "youtube") {
    ...
    <div className="space-y-5">
      ...
      <Card title="외부 API 테스트">
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {[youtubeResult, llmResult].map((result, index) =>
            result ? (
